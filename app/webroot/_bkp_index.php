...
<body> 
    ...
    //////////////////////////////////////////// FSockOpen //////////////////////////////
        ...
        $host = ($url_parts['scheme'] == 'https' ? 'ssl://' : '').'localhost';
	...
	$array_counter 			= 0;
	$getcontent_to_replace	= array();
	$getcontent_result 		= array();

	function contruct_page($page, $archive){

		global $body;

		$file 	= explode(DIRECTORY_SEPARATOR, dirname(__FILE__));
		array_pop($file);
		$file 	= implode(DIRECTORY_SEPARATOR, $file);
		$file 	= $file.DS.'view'.DS.'pages'.DS.$page.DS.'index.php';
		load($file, '');
		$source = $body;

		

		if(!empty(preg_match_all("'<getcontent_content>(.*?)</getcontent_content>'si", $source, $match))){

			preg_match_all("'<getcontent_content>(.*?)</getcontent_content>'si", $source, $match); $content = $match[0];

			//$var = (implode($content));
			//die();

			preg_match_all("'<div class=\"getcontent_table\">(.*?)</div>'si", $source, $match); $table = $match[1];
			$table_value_array = array();
			foreach ($table as $table_value) {
				$table_value_array[] = $table_value;
			}

			preg_match_all("'<div class=\"getcontent_where\">(.*?)</div>'si", $source, $match); $where =  $match[1];
			$where_value_array = array();
			foreach ($where as $where_value) {
				$where_value_array[] = $where_value;
			}

			preg_match_all("'<div class=\"getcontent_extras\">(.*?)</div>'si", $source, $match); $extras = $match[1];
			$extras_value_array = array();
			foreach ($extras as $extras_value) {
				$extras_value_array[] = $extras_value;
			}

			preg_match_all("'<div class=\"getcontent_order\">(.*?)</div>'si", $source, $match); $order = $match[1];
			$order_value_array = array();
			foreach ($order as $order_value) {
				$order_value_array[] = $order_value;
			}

			preg_match_all("'<div class=\"getcontent_ascdesc\">(.*?)</div>'si", $source, $match); $ascdesc = $match[1];
			$ascdesc_value_array = array();
			foreach ($ascdesc as $ascdesc_value) {
				$ascdesc_value_array[] = $ascdesc_value;
			}

			preg_match_all("'<div class=\"getcontent_limit\">(.*?)</div>'si", $source, $match); $limit = $match[1];
			$limit_value_array = array();
			foreach ($limit as $limit_value) {
				$limit_value_array[] = $limit_value;
			}

			global $getcontent_to_replace;
			global $getcontent_result;
			global $items2content;

			$x = 0;
			foreach ($content as $value) {
				$table 		= $table_value_array[$x];
				$where 		= $where_value_array[$x];
				$extras 	= $extras_value_array[$x];
				$order 		= $order_value_array[$x];
				$ascdesc 	= $ascdesc_value_array[$x];
				$limit 		= $limit_value_array[$x];
				//foreach_test($value, $table);
				...
				$getcontent_to_replace[] = $value;
				

			}

			$final = str_replace($getcontent_to_replace, $getcontent_result, $source);

			/*
			preg_match_all( "'php(.*?)/php'si", $final, $match );  $final_content = $match[1];

			
			global $str;
			global $final_content_array;
			$final_content_to_replace[] = array();
			$final_content_array = array();

			foreach ($final_content as $value) {
				
				evalfunc($value);
				$final_content_to_replace[] = $value;
				$final_content_array[] = $str;
			}

			$result = str_replace($final_content_to_replace, $final_content_array, $source);
